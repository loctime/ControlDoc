...
/**
 * Obtener documentos requeridos de todos los tenants
 */
async function obtenerDocumentosRequeridos() {
  try {
    ...
    // Verificar que tenants es un array
    if (!Array.isArray(tenants)) {
      console.error('❌ Tenants no es un array:', typeof tenants, tenants);
      return [];
    }
    ...
    for (const tenantId of tenants) {
      try {
        console.log(`🔍 Obteniendo documentos requeridos del tenant: ${tenantId}`);
        const docs = await obtenerDocumentosDeTenant(tenantId, 'requiredDocuments');
        console.log(`📄 Encontrados ${docs.length} documentos en tenant ${tenantId}`);
        todosLosDocs.push(...docs);
      } catch (error) {
        console.error(`❌ Error obteniendo documentos requeridos del tenant ${tenantId}:`, error);
      }
    }
    
    console.log(`📊 Total de documentos requeridos obtenidos: ${todosLosDocs.length}`);
    return todosLosDocs;
  } catch (error) {
...
/**
 * Obtener documentos subidos de todos los tenants
 */
async function obtenerDocumentosSubidos() {
  try {
    ...
    for (const tenantId of tenants) {
      try {
        console.log(`🔍 Obteniendo documentos subidos del tenant: ${tenantId}`);
        const docs = await obtenerDocumentosDeTenant(tenantId, 'uploadedDocuments');
        console.log(`📄 Encontrados ${docs.length} documentos en tenant ${tenantId}`);
        todosLosDocs.push(...docs);
      } catch (error) {
        console.error(`❌ Error obteniendo documentos subidos del tenant ${tenantId}:`, error);
      }
    }
    
    console.log(`📊 Total de documentos subidos obtenidos: ${todosLosDocs.length}`);
    return todosLosDocs;
  } catch (error) {
 ...
 */
async function obtenerDocumentosDeTenant(tenantId, collectionName) {
  try {
    ...
    console.error(`❌ Error obteniendo documentos del tenant ${tenantId}, colección ${collectionName}:`, error);
    return [];
  }
}
